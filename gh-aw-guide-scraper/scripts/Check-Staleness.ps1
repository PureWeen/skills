...
function Get-GitHubLatestRelease {
    ...
    try {
        # Use /latest endpoint which only returns non-prerelease, non-draft releases
        $json = gh api "repos/$Repo/releases/latest" --jq '{tag_name: .tag_name, name: .name, published_at: .published_at, body: .body}' 2>&1
        if ($LASTEXITCODE -ne 0) {
            return @{
                status = 'error'
                repo   = $Repo
...
function Get-GitHubReleasesSince {
    ...
    try {
        # Fetch recent releases (up to 10), excluding pre-releases and drafts
        $json = gh api "repos/$Repo/releases?per_page=20" --jq '[.[] | select(.prerelease == false and .draft == false) | {tag_name: .tag_name, name: .name, published_at: .published_at, body: .body}] | .[:10]' 2>&1
        if ($LASTEXITCODE -ne 0) {
            return @{
                status = 'error'
                repo   = $Repo
        ...
        switch ($source.type) {
            'issue' {
                ...
                if ($result.status -eq 'ok') {
                    ...
                    $expectedState = if ($resExpected) { 'closed' } else { 'open' }
                    $stateMatch = $result.state -eq $expectedState
                    $stateEmoji = if ($stateMatch) { '✅' } else { '⚠️' }
                    $stateColor = if ($stateMatch) { 'Green' } else { 'Yellow' }
                    Write-Host " $stateEmoji $($result.state) (expected: $expectedState)" -ForegroundColor $stateColor
                }
                else {
