...
| **Container image digest pinning** (v0.70.0+; restored v0.79.6) | Lock files pin built-in container images by digest for reproducible, tamper-resistant execution. Digest pinning for AWF firewall sidecar images was restored in v0.79.6 after a temporary regression — recompile to pick up the latest pinned digests. | Only covers images managed by `gh aw compile` — custom containers are not auto-pinned |
| **Go MCP server process guardrail** (v0.79.6) | The Go MCP server enforces a 4-process limit on child `gh` invocations, preventing runaway process trees and improving resource safety on constrained runners | Does not limit direct subprocess creation by the agent container itself |
...
tools:
  github:
    min-integrity: approved        # Explicit setting — controls content visibility
...
When `min-integrity` is omitted, the runtime's `determine-automatic-lockdown` step applies appropriate integrity levels based on event type, actor trust level, and repository context.
...
- **Checks:**
  ...
  - [ ] Set `min-integrity` intentionally, or omit to rely on automatic runtime lockdown
