...
**Runaway-loop guardrail:** `max-turn-cache-misses` (top-level) caps the number of consecutive AWF API-proxy cache misses before the proxy blocks further model requests — a backstop against an agent stuck in a non-progressing loop that keeps issuing uncached calls. It maps to `apiProxy.maxCacheMisses`; a frontmatter value overrides the engine default.
...
- gh-aw emits spans following the OpenTelemetry **`cicd.automation.*`** semantic conventions and propagates a W3C **`TRACEPARENT`** through every engine step, so child OpenTelemetry SDKs/tools join the same trace rather than starting disconnected ones. Cross-workflow parent/child correlation still flows through `aw_context`; see *OpenTelemetry / OTLP Observability Flow* in [`references/architecture.md`](references/architecture.md).
...
- **`safe-outputs.mentions:` authorization gate** — Globally controls *who* may be @-mentioned by any safe output (distinct from per-output `allowed-mentions` escaping). `allowed-collaborators: true` (default) permits repo collaborators; `allow-context: true` permits event-context participants; `allowed:` lists always-allowed users/bots; `allowed-teams:` authorizes whole GitHub teams (`org/team-slug` or bare `team-slug`; bot accounts excluded). **`allowed-teams` needs a token with `read:org` scope** — the default `GITHUB_TOKEN` lacks it, so supply a classic/fine-grained PAT or GitHub App token with Members:read, or team lookups fail HTTP 403/404 (the run continues without those members). `allow-team-members:` is a deprecated alias for `allowed-collaborators:` — run `gh aw fix --write` to migrate.
...
- **`safe-outputs.max-patch-size:`** — Caps the git diff size for PR-producing safe outputs (`create-pull-request`, `push-to-pull-request-branch`); the job fails closed if the patch exceeds it. Default is **4096 KB (4 MB)** (range 1–10,240 KB); a per-output `max-patch-size` overrides it for one output type. Lower it on workflows that should only make small, reviewable edits.
...
- **`normalize-closing-keywords: true`** — On `create-issue`, `add-comment`, and `create-pull-request`, strips wrapping backticks from recognized issue-closing keywords in body text (e.g., `` `Closes #123` `` → `Closes #123`) so GitHub processes them as real closing keywords instead of inert code spans.
...
- **Billing & cost-management reference** — [`gh-aw billing docs`](https://github.github.com/gh-aw/reference/billing/)
